    ...
    def get_1x_lr_params(self):
        ...
        for i in range(len(modules)):
            for m in modules[i].named_modules():
                if self.freeze_bn:
                    if isinstance(m[1], MetaConv2d):
                        for p in m[1].parameters():
                            if p.requires_grad:
                                yield p
                else:
                    if isinstance(m[1], MetaConv2d) or isinstance(m[1], SynchronizedBatchNorm2d) \
                            or isinstance(m[1], MetaBatchNorm2d):
                        for p in m[1].parameters():
                            if p.requires_grad:
                                yield p
